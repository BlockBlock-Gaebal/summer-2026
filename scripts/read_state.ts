...
function renderSnapshot(snap: ChallengeSnapshot): string {
  ...
  const alpha = Number(c.alphaBp) / 10000;
  lines.push('');
  lines.push('═══ 방 정보 ═══');
  lines.push(`  Challenge   : ${c.id}`);
  lines.push(`  상태        : ${c.statusLabel} (${c.status})`);
  lines.push(`  진행        : day ${c.currentDay} / ${c.totalDays}`);
  lines.push(`  α           : ${alpha} (alpha_bp=${c.alphaBp})`);
  lines.push(`  vault       : ${mistToSui(c.vault)} SUI (${c.vault} MIST)`);
  lines.push(`  daily_drip  : ${mistToSui(c.dailyDrip)} SUI (${c.dailyDrip} MIST)`);
  lines.push(`  오라클      : ${c.oracle}`);
  lines.push(`  Table       : ${c.participantsTableId}`);
  lines.push(`  참가자 수   : ${c.participantsSize}`);

  lines.push('');
  lines.push('═══ 참가자 ═══');
  if (participants.length === 0) {
    lines.push('  (참가자 없음)');
    lines.push('  ⚠️ 빈 방이다. 이 상태로 submit_results를 호출하면 전멸로 판정되어');
    lines.push('     즉시 ENDED로 잠기고 되돌릴 수 없다. 참가자를 채운 뒤 진행할 것.');
  } else {
    const head =
      '  ' +
      '주소'.padEnd(16) +
      'stake(SUI)'.padStart(12) +
      'start'.padStart(7) +
      'failed_day'.padStart(12) +
      'claimable(SUI)'.padStart(16) +
      '  상태';
    lines.push(head);
    lines.push('  ' + '─'.repeat(head.length + 4));

    for (const p of participants) {
      const alive = p.failedDay === 0n;
      const state = alive
      ...
      lines.push(
        '  ' +
          shortAddress(p.address).padEnd(16) +
          mistToSui(p.stake).padStart(12) +
          String(p.startDay).padStart(7) +
          String(p.failedDay).padStart(12) +
          mistToSui(p.claimable).padStart(16) +
          '  ' +
          state,
      );
    }

    const totalStake = participants.reduce((a, p) => a + p.stake, 0n);
    const totalClaimable = participants.reduce((a, p) => a + p.claimable, 0n);
    lines.push('  ' + '─'.repeat(head.length + 4));
    lines.push(
      '  ' +
        '합계'.padEnd(16) +
        mistToSui(totalStake).padStart(12) +
        ''.padStart(7) +
        ''.padStart(12) +
        mistToSui(totalClaimable).padStart(16),
    );
  }
...
async function main(): Promise<void> {
  ...
  const challengeId = process.argv[2] ?? process.env.CHALLENGE_ID;
  if (!challengeId) {
    console.error(
      [
        '사용법: npm run read-state -- <CHALLENGE_ID>',
        '   또는: scripts/.env 에 CHALLENGE_ID=0x... 를 넣고 npm run read-state',
        '',
        'Challenge ID의 정본은 docs/DEPLOYMENT.md 다 (D-20).',
      ].join('\n'),
    );
    process.exit(1);
  }
  ...
  const url = process.env.SUI_GRPC_URL;
  const client = createSuiClient({ network, url });

  try {
    const snap = await readState(client, challengeId);
    console.log(renderSnapshot(snap));
  } catch (e) {
    console.error(`\n조회 실패: ${humanizeError(e)}\n`);
    process.exit(1);
  }
}
