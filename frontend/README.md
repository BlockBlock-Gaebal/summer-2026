...
> 프로젝트 생성부터 참가자 표가 화면에 뜰 때까지 순서대로 따라가면 된다.
> 이 문서는 그 다음에 읽는 참조용이다.
>
> 이 파트의 문서는 셋이고 성격이 다르다.
>
> | 문서 | 성격 |
> |---|---|
> | [`QUICKSTART.md`](./QUICKSTART.md) | **시작점.** 빈 상태에서 화면이 뜰 때까지의 단계별 절차 |
> | [`FRONTEND_GUIDE.md`](./FRONTEND_GUIDE.md) | **상세 인수인계 문서.** 이미 구현된 대시보드의 구조·실행법·파일 구성. 이어서 작업하거나 남의 코드를 넘겨받을 때 본다 |
> | 이 문서 | 파트 개요와 착수 전 함정 (참조용) |
