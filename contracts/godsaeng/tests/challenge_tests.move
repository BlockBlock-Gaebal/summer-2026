...
fun test_join_three_vault_is_30_sui() {
    ...
    challenge::create_challenge(5, scenario.ctx()); // 5일짜리 챌린지 방 생성
...
fun setup_abc(scenario: &mut ts::Scenario, total_days: u64) {
    challenge::create_challenge(total_days, scenario.ctx());
...
// ============================================================
// ver2 — 가변 베팅 + 지분 가중 분배
// 스펙 §3: A=20, B=10, C=30, D=10 (총 70) / B day2, C day4 탈락
// 기대값: A = 20 + 40×20/30 = 46.66..., D = 10 + 40×10/30 = 23.33...
// ============================================================

#[test]
fun test_ver2_weighted_distribution() {
    let mut scenario = ts::begin(ORACLE);
    challenge::create_challenge(5, scenario.ctx());
    join_as(&mut scenario, A, 20 * ONE_SUI);
    join_as(&mut scenario, B, 10 * ONE_SUI);
    join_as(&mut scenario, C, 30 * ONE_SUI);
    join_as(&mut scenario, D, 10 * ONE_SUI);

    submit_as(&mut scenario, ORACLE, vector[]);  // day1
    submit_as(&mut scenario, ORACLE, vector[B]); // day2: B 탈락 (몰수 10)
    submit_as(&mut scenario, ORACLE, vector[]);  // day3
    submit_as(&mut scenario, ORACLE, vector[C]); // day4: C 탈락 (몰수 30)
    submit_as(&mut scenario, ORACLE, vector[]);  // day5
    finalize_as(&mut scenario, ORACLE);

    // 몰수 40 SUI를 성공자 지분(A:D = 20:10) 비례 분배 — MIST 정수 나눗셈(floor)
    // A = 20e9 + floor(40e9×20e9/30e9) = 46,666,666,666
    // D = 10e9 + floor(40e9×10e9/30e9) = 23,333,333,333
    assert!(claim_as(&mut scenario, A) == 46_666_666_666);
    assert!(claim_as(&mut scenario, D) == 23_333_333_333);

    scenario.next_tx(ORACLE);
    let ch = scenario.take_shared<Challenge>();
    // 보존 법칙: 46,666,666,666 + 23,333,333,333 + dust = 70 SUI → dust 1 MIST
